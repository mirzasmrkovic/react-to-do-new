...
class TaskSlide extends Component {
  ...
  componentDidMount() {
    setTimeout(() => {
      ...
      setTimeout(() => this.props.closeTaskSlide(), 400)
    }, 3000)
  }
  render() {
    return (
      <div onClick={this.props.closeTaskSlide} className={(this.state.closeSlide ? 'slide-up' : 'slide-down') + ' task-slide title-s bold-title uppercase flex-property justify-content-between align-items-center'}>
